...
public class GUIManager : MonoBehaviour
{
    ...
    private bool ObjInteractionsVisu;
    ...
    private void ReplayGUI()
    {
        ...
        config.NewProject(curReplayProject);
        ...
        config.NewSession(curReplaySession);
        if (curReplaySession != replayManager.replaySession || needReplayListUpdate)
        {
            ...
            foreach (string file in replayManager.GetDataFiles(root,"objectsdata"))
        ...
        bool newObjInteractionsVisu = GUILayout.Toggle(ObjInteractionsVisu, " Object Interaction Visualization", stylesManager.toggleStyle);
        if(newObjInteractionsVisu != ObjInteractionsVisu)
        {
            ObjInteractionsVisu=newObjInteractionsVisu;
            replayManager.ActivateObjInteractionVisu();
        }
        ...
        bool newAugmentData = GUILayout.Toggle(augmentData, " Data Augmentation", stylesManager.toggleStyle);
        if (newAugmentData != augmentData)
        {
            augmentData = newAugmentData;
            replayManager.setAugmentData();
        }
    }
    ...
    private void AsyncLoad()
    {
        ...
        replayManager.ReadActionsDataFile(root,recId);
